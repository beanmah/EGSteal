...
def mol_to_graph_data_obj_simple(mol):
    ...
    if mol is None or mol.GetNumAtoms() == 0:
        print("Warning: Invalid molecule with no atoms")
        return Data(
    ...
    for atom in mol.GetAtoms():
        try:
            ...
            print(f"Warning: Invalid atom feature for atom {atom.GetIdx()}: {e}")
            continue
    if not atom_features_list:
        print("Warning: No valid atoms found")
        return Data(
            x=torch.empty((0, 2), dtype=torch.long),
            edge_index=torch.empty((2, 0), dtype=torch.long),
            edge_attr=torch.empty((0, 2), dtype=torch.long)
    ...
    if len(mol.GetBonds()) > 0:  # mol has bonds
        for bond in mol.GetBonds():
            ...
            if i >= num_atoms or j >= num_atoms or i < 0 or j < 0:
                print(f"Warning: Invalid bond indices: i={i}, j={j}, num_atoms={num_atoms}")
                continue
            try:
                edge_feature = [
                ...
                print(f"Warning: Invalid bond feature for bond {bond.GetIdx()}: {e}")
                continue
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol is not None:
                    try:
                        ...
                        print(f"Error processing SMILES at index {i}: {smiles_list[i]}, Error: {e}")
            ...
            if invalid_smiles_count > 0:
                print(f"Total invalid SMILES: {invalid_smiles_count}/{len(smiles_list)}")

            if not data_list:
                raise ValueError("No valid SMILES found in Tox21 dataset")
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol is not None:
                    try:
                        ...
                        print(f"Error processing SMILES at index {i}: {smiles_list[i]}, Error: {e}")
                        invalid_smiles.append(smiles_list[i])
                        invalid_smiles_count += 1
                else:
                    print(f"Invalid SMILES at index {i}: {smiles_list[i]}")
                    invalid_smiles.append(smiles_list[i])
            ...
            if not data_list:
                raise ValueError("No valid SMILES found in HIV dataset")
